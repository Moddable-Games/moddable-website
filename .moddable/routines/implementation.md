...
When generating a `templates/shell.html` for a new game entry, read an existing `shell.html` (e.g. `games/draughts/templates/shell.html`) first and match it exactly — favicon path, CSS link order, JS references. Do not template from memory.
...
### Incomplete variant content — raise a follow-up issue

If source content is embedded for some variants but incomplete or missing for others (e.g. a property list is referenced but not embedded), do not leave the variant file partially implemented. Either:
- Omit the variant file entirely and raise a new `research` issue for it with a clear description of what source content is needed, OR
- Implement the variant with a clearly marked placeholder section and raise the follow-up issue

Never silently deliver an incomplete variant without raising a follow-up issue in the same run.
...
games/{slug}/
  ...
  diagrams/
    svg/
      {slug}-board.svg    ← see Board diagrams section below
  logos/
    {slug}-logo.svg       ← see Logo section below
  og-variants/
  pdf/
  templates/
    shell.html
...
Create all directories. Use `.gitkeep` only for directories that are genuinely empty with no deliverable — `og-variants/`, `pdf/`, `templates/` (if shell.html is not yet created). Do not use `.gitkeep` for `diagrams/svg/` or `logos/` — these always have deliverables (see below).

### Board diagrams

Every new game entry requires at least one board diagram SVG in `diagrams/svg/`. The diagram must be present and renderable before the issue can be closed — an empty placeholder directory is not acceptable.

**For chess, draughts, go, morris, dungeon-chess, and royal-ur family entries:** diagrams are generated by `moddable-chess/scripts/generate-rules-boards.js`. Add the new game config to that script and run it to produce the SVG files.

**For all other game families** (cross-and-circle, property boards, tile games, etc.): there is no automatic renderer. Produce the board diagram as a handcrafted SVG file directly. The SVG must:
- Accurately represent the board layout described in the source material
- Use the game's theme colours from `theme.css`
- Include a title and brief legend where relevant
- Meet the readability standard: minimum 10px font size for any text, minimum 18px for colour swatches or legend icons. Never use 6px text or similarly unreadable sizes.
- Be sized so it renders clearly at typical rulebook page widths (400–700px wide)

A diagram that exists and is readable is the standard. A missing or empty diagram directory means the issue is not done.

### Logo

Every new game entry requires a logo file in `logos/`. Existing entries use PNG logos; SVG is also acceptable. The logo appears on homepage cards — without it the card renders without an icon.

Produce a simple SVG logo that represents the game visually. It does not need to be elaborate — a clean icon using the game's theme colour is sufficient. Name it `{slug}-logo.svg`. Do not leave the `logos/` directory empty.
...
Before merging, verify each item explicitly:

1. **No AI attribution anywhere.** Search every file you created or modified for: "Claude", "Claude Code", "Generated by", "Co-authored-by". If found anywhere — in file content, commit messages, PR description, or issue comments — remove it before merging. This includes PR descriptions: write them from scratch, do not auto-generate footers.
2. **PR description ends with substantive content.** The final line must describe what was built, not a signature or attribution line.
...
Only merge when the implementation genuinely meets the quality bar and pre-merge checklist above.
