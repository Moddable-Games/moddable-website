import { T, CATEGORY_COLORS, el, url, HEX_BG } from './mg-core.js';
import { track } from './mg-analytics.js';

export function modCard({ category, title, baseGame, stats, body, href = '#', source = '', logo = '' }) {
  ...
  card.addEventListener('click', (e) => {
    ...
    track('select_content', { content_type: 'mod', item_id: title, category: category });
  });
  return card;
}

export function pageHero({ eyebrow, title, lede, accent = T.cosmicGlow, withHorizon = false, minHeight = 420 }) {
  ...
  if (withHorizon) {
    ...
    sec.appendChild(el('div', { class:'page-hero__hex-layer', style:`background-image:${HEX_BG}` }));
