import { T, el, url, data } from './mg-core.js';
import { linkBtn } from './mg-buttons.js';
import { modCard } from './mg-cards.js';
import { navbar } from './mg-navbar.js';
import { footer } from './mg-footer.js';
import { MOD_PAGES } from './mg-mods-content.js';
import { initReveal, initTocSpy } from './mg-animations.js';

var pathParts = window.location.pathname.replace(/\/$/, '').split('/');
var slug = pathParts[pathParts.length - 1];

if (slug) {
  var page = MOD_PAGES[slug];
  if (page) {
    ...
    function init() {
      ...
      data.get('mods').then(function(mods) {
    ...
    function renderHero(listing) {
      ...
      hex.style.backgroundImage = "url('" + url('/img/' + (HEX_GRIDS[accent] || HEX_GRIDS.blue)) + "')";
      ...
      var backLink = el('a', {href: url('/mods/'), class: 'game-hero__back'});
      ...
      catTag.style.background = T[accent];
      ...
      versionTag.style.color = T[accent === 'green' ? 'green' : accent === 'red' ? 'redBright' : 'cosmicGlow'];
