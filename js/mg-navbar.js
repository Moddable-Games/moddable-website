...
import { linkBtn } from './mg-buttons.js';
...
export function navbar(activeId) {
  const NAV_ITEMS = [
    ...
    { id:'Tools', href:url('/tools/'), accent:'#3a9928', children:[['Twilight',url('/tools/ti/')],['Talisman',url('/tools/talisman/')],['Nukes',url('/tools/nukes/')],['Dice',url('/tools/dice/')],['Decks',url('/tools/decks/')],['Chess',url('/tools/chess/')],['Oracles',url('/tools/oracles/')]] },
    { id:'Developers', href:url('/developers/'), accent:'#8b5cf6', children:[['Tools API',url('/developers/api/')],['Examples',url('/developers/examples/')],['Live Server','https://tools.moddable.games/']] },
  ...
  for (const item of NAV_ITEMS) {
    ...
    a.addEventListener('click', function() { track('nav_click', { nav_item: item.id, nav_source: 'desktop' }); });
    if (isActive) {
  ...
  searchBtn.addEventListener('click', () => { if (window.__mgOpenSearch) window.__mgOpenSearch(); });
  ...
  hamburger.addEventListener('click', () => {
    ...
    if (drawerOpen) track('nav_drawer_open');
  });
  drawer.addEventListener('click', (e) => {
    if (e.target.tagName === 'A') {
      track('nav_click', { nav_item: e.target.textContent, nav_source: 'drawer' });
      closeDrawer();
    }
  });
