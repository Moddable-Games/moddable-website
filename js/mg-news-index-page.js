import { T, el, url, data, track, linkBtn, pageHero, navbar, footer, sectionHero, buildHeroFeature } from './mg.js';
...
lede: 'Essays, announcements, and build logs. Written by humans, for humans.',
feature: buildHeroFeature('news')
}));

data.load(['news']).then(function(store) {
...
renderTopics();
renderArchive();
if (track) track('filter_select', { filter_type: cat !== 'All' ? 'topic' : 'month', filter_value: cat !== 'All' ? cat : month || 'all', page: 'news' });
}
...
if (searchVal.length >= 3) {
  newsSearchDebounce = setTimeout(function() { if (track) track('search', { search_term: searchVal, page: 'news' }); }, 800);
}
});
