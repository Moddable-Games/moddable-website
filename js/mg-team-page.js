import { el, url, data, footer, sectionHero, navbar } from './mg.js';
...
var grid = document.getElementById('team-grid');
  ...
  data.load(['team']).then(function(store) {
    store.team.forEach(function(m) {
      var card = el('a', {class:'team-card', href: url('/team/' + m.handle + '/')});
      ...
      if (m.socials) {
        var socials = el('div', {class:'team-card__socials'});
        ['linkedin', 'x', 'instagram'].forEach(function(platform) {
          if (m.socials[platform]) {
            var link = el('a', {href: m.socials[platform], class: 'team-card__social', target: '_blank', rel: 'noopener'});
