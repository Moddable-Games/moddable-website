import { HEX_BG, el, data, track, btn } from './mg.js';
import * as State from './mg-ti-dash-state.js';
import { renderPanel as renderObjectivesPanel } from './mg-ti-dash-objectives.js';
import { renderPanel as renderAgendaPanel, loadAgendas } from './mg-ti-dash-agenda.js';
let root = null;
...
function render(container) {
  ...
  hexBg.style.backgroundImage = HEX_BG;
...
function renderVictoryScreen(state) {
  ...
  actions.appendChild(el('button', { class: 'td-victory__new-game', onClick: () => { State.reset(); import('./mg-ti-dash-setup.js').then(m => m.render(root)); } }, 'New Game'));
...
function renderAgendaBody(state) {
  ...
  main.appendChild(renderAgendaPanel(state));
...
function renderSidePanel(state) {
  ...
  side.appendChild(renderObjectivesPanel(state));
  if (state.phase === 'agenda' && state.mecatolClaimed) {
    side.appendChild(renderAgendaPanel(state));
  }
  return side;
}
...
function confirmReset() {
  if (confirm('End this game session?')) {
    State.reset();
    import('./mg-ti-dash-setup.js').then(m => m.render(root));
  }
}
