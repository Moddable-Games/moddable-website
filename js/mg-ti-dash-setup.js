...
window.MG_TIDash_Setup = (() => {
  ...
  function renderStep1() {
    ...
    if (State.load()) {
      ...
      resumeRow.appendChild(el('button', { class: 'td-setup__resume-btn', onClick: () => { if (window.MG_TIDash_Score) window.MG_TIDash_Score.render(container); } }, 'Resume Previous Game'));
  ...
  function startGame() {
    ...
    initObjectivePools();
    if (window.MG_TIDash_Agenda) window.MG_TIDash_Agenda.loadAgendas(cfg.expansions);
    if (window.MG_TIDash_Score) window.MG_TIDash_Score.render(container);
  }
  ...
  return { render, loadData, PLAYER_COLORS };
})();
