import { T, el, data, track, btn, navbar, footer, sectionHero } from './mg.js';
...
TABS.forEach(t => {
  ...
  b.addEventListener('click', () => {
    ...
    if (track) track('dice_tab_select', { tab: t.id });
  });
  wrap.appendChild(b);
});
}
...
function rollDice() {
  if (track) track('dice_roll', { die_type: 'd' + selectedDie, dice_count: diceCount, modifier: modifier });
...
if (track) track('dice_simulation', { tab: activeTab, iterations: ITERATIONS });
...
function rollPipeline() {
  ...
  if (track) track('dice_roll', { die_type: 'w40k', attacks: attacks });
}
...
function runSim() {
  ...
  if (track) track('dice_simulation', { tab: 'w40k', iterations: ITERATIONS });
}
